...
def save_test_result(report_dir, test_name, frame, ref_path, out_img, diff_img, stats):
    ...
    # Copy the ref image so the report is self-contained
    if ref_path is not None:
        shutil.copy2(ref_path, op.join(img_dir, f"{key}_ref.png"))
    ...
    result = {"key": key, "test_name": test_name, "frame": frame, "has_images": has_diff, "has_ref": has_ref, **stats}
    _atomic_write_text(op.join(data_dir, f"{key}.json"), json.dumps(result))


def _build_stats_html(r):
    if r.get("missing_ref"):
        return "<b>reference file missing</b>"
    parts = []
    ...
    if total_pixels > 0:
        ...
        parts.append(f"diff: {diff_pct:.3f}% / {threshold_pct:.3f}% ({diff_count}/{total_pixels})")
    if max_diff > 0:
        parts.append(f"max channel diff: {max_diff} / {tolerance}")
    if aa_count > 0:
        parts.append(f"aa (ignored): {aa_count} / {total_pixels}")
...
def _prepare_result(r, report_dir):
    ...
    has_ref = r.get("has_ref", False)

    missing_ref = r.get("missing_ref", False)
