...
cdef extern from "nopegl/nopegl.h":
    ...
    ngl_scene *ngl_scene_duplicate(ngl_scene *s)
    ...
    cdef struct ngl_node_funcs:
        ...
        int (*init_resources)(void *reserved, void *user_data)
        ...
        void (*free)(void *reserved, void *user_data)
    ...
    int ngl_effect2dshader_set_range(ngl_node *node, double start, double end)
...
cdef extern from "nopegl/nopegl_opengl.h":
    ...
    int ngl_custom_texture_set_texture_info_gl(ngl_node *node, const ngl_custom_texture_info_gl *info)
...
cdef class _Node:
    ...
    def _duplicate(self, bint duplicate_resources=False):
        cdef uint32_t flags = 0
        if duplicate_resources:
            flags |= NGL_NODE_DUPLICATE_RESOURCES
    ...
    def _effect2dshader_set_range(self, double start, double end):
        return ngl_effect2dshader_set_range(self.ctx, start, end)
...
cdef class Scene:
    ...
    def duplicate(self):
        cdef ngl_scene *dup = ngl_scene_duplicate(self.ctx)
        if dup is NULL:
            raise MemoryError()
        cdef Scene scene = Scene.__new__(Scene)
        ngl_scene_unrefp(&scene.ctx)
        scene.ctx = dup
        cdef const ngl_scene_params *params = ngl_scene_get_params(dup)
        scene.root = _Node(ctx=<uintptr_t>params.root)
        return scene
...
cdef int _py_custom_texture_init_resources(void *reserved, void *user_data) noexcept:
    cdef pystate.PyGILState_STATE gil_state = pystate.PyGILState_Ensure()
    node = <CustomTexture>user_data
    ret = _wrap_func(node._init_resources)
    pystate.PyGILState_Release(gil_state)
    return ret
...
cdef class CustomTextureInfoGL:
    cdef ngl_custom_texture_info_gl info

    def __cinit__(self):
        memset(&self.info, 0, sizeof(self.info))

    def __init__(self, int texture, int target, int width, int height):
...
cdef class CustomTexture(_Node):
    ...
    def __init__(
        ...
        funcs.init_resources = _py_custom_texture_init_resources
    ...
    def set_texture_info_gl(self, info: Optional[CustomTextureInfoGL]):
        cdef uintptr_t ptr = 0
        cdef ngl_custom_texture_info_gl *texture_info = NULL
        if info is not None:
            ptr = info.cptr
            texture_info = <ngl_custom_texture_info_gl *>ptr
        cdef int ret = ngl_custom_texture_set_texture_info_gl(self.ctx, texture_info)
        if ret < 0:
