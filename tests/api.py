...
def api_param_get(width=256, height=256):
    ...
    rect = ngl.DrawRect(rect=(10, 20, 100, 80), fill=fill)
    ...
    # i32 getter
    canvas = ngl.Canvas(children=[rect], width=320, height=240)
    assert canvas.get_width() == 320
    assert canvas.get_height() == 240

    # str getter (label is a base node param)
    labeled = ngl.DrawRect(rect=(0, 0, 10, 10), fill=fill, label="my-rect")
