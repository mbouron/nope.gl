...
float randomLine(float seed, vec2 uv)
{
    ...
    return mix(0.85, 1.0, l);
}

// Generate some blotches.
float randomBlotch(float seed, vec2 uv, vec2 size)
{
    float x = rand(seed);
    ...
    return mix(0.4 + 0.2 * (1.0 - (s / 0.02)), 1.0, v);
}
...
void main()
{
    ...
    float t = float(int(s_time * FREQUENCY));
    
    // Get some image movement
    vec2 suv = uv + 0.002 * vec2( rand(t), rand(t + 23.0));
    
    // Get the image
    vec3 image = texture2D( u_texture, vec2(suv.x, suv.y) ).xyz;
    ...
    vI *= pow(16.0 * uv.x * (1.0-uv.x) * uv.y * (1.0-uv.y), 0.2);
    ...
    if ( 0 < s ) vI *= randomBlotch( t+6.0+19.* float(0), uv, u_size);
    if ( 1 < s ) vI *= randomBlotch( t+6.0+19.* float(1), uv, u_size);
    if ( 2 < s ) vI *= randomBlotch( t+6.0+19.* float(2), uv, u_size);
    if ( 3 < s ) vI *= randomBlotch( t+6.0+19.* float(3), uv, u_size);
    if ( 4 < s ) vI *= randomBlotch( t+6.0+19.* float(4), uv, u_size);
    if ( 5 < s ) vI *= randomBlotch( t+6.0+19.* float(5), uv, u_size);

    #endif

    // Show the image modulated by the defects
    vec3 col = oldImage * vI;
