...
void main()
{
    ...
    // mirroring
    //uv.y = fract(scale * uv.y - 0.5 * (scale - 1.));
    p.y = 2. * (fract(0.5 + 0.5 * y_scale * p.y) - 0.5);
    ...
    float radiusNotch = 2e-2;
    
    vec2 sizeNotch = vec2(0.04, 0.16) - radiusNotch;
    vec2 offsetP2 = vec2(ratio - .5 * paddingH, 0.) + 5e-3 * vec2(noise1d(20. + 10. * s_time), noise1d(30. + 10. * s_time));
