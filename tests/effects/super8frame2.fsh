...
void main()
{
    ...
    p.y = 2. * (fract(0.5 + 0.5 * scale * p.y) - 0.5);
    ...
    float radiusNotch = 4e-2;
    
    vec2 sizeNotch = vec2(0.08, 0.16) - radiusNotch;
    vec2 offsetP2 = vec2(ratio - sizeNotch.x, 0.) + 5e-3 * vec2(noise1d(20. + 10. * s_time), noise1d(30. + 10. * s_time));
