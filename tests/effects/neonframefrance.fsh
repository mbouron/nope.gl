...
void main()
{
   ...
   // float time_increment = floor(mod(4.0 * i_time, 6.0));
    ...
    float y_length = 1e-1;
    //float uv_xy = mix(1.0 - uv.x, uv.y, 0.9);
    float y_ramp_bot = smoothstep(0.0, 1.0, (uv.y - (0.33 - 0.5 * y_length)) / y_length);
    float y_ramp_top = smoothstep(0.0, 1.0, (uv.y - (0.67 - 0.5 * y_length)) / y_length);
    
    vec3 red = hsl2rgb(0.95, 1.0, 0.4);
    vec3 blue = hsl2rgb(0.65, 1.0, 0.4);
    vec3 white = vec3(1.0);
    vec3 neon_glow_color = mix(mix(red, white, y_ramp_bot), blue, y_ramp_top);//hsl2rgb(0.65 + 0.3 * y_ramp, 1.0, 0.4);
