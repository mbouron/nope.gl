...
void main() {
    ...
    //uv.y = 1.0 - uv.y;
    vec3 col;
    float x = sin(0.3*s_time+uv.y*21.0)*sin(0.7*s_time+uv.y*29.0)*sin(0.3+0.33*s_time+uv.y*31.0)*0.0017;
    ...
    float scans = clamp(0.35+0.35*sin(3.5*s_time+uv.y*u_size.y*1.5), 0.0, 1.0);
    
    float s = pow(scans,0.7);
    col = col*vec3(0.4+0.7*s);
    
    col *= 1.0+0.01*sin(110.0*s_time);
