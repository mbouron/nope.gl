...
void main()
{
    ...
    //float time_increment = floor(mod(2.0 * i_time, 6.0));
    
    vec3 neon_glow_color = hsl2rgb(mod(0.2 - 0.5 * uv.y, 1.0), 1., 0.5);
...
//    vec3 neon_color_inner = mix(neon_glow_color, vec3(1.), inner_color_mix_factor);
    ...
    float time_factor = mix(1.0, mix(noise1d(i_time), noise1d(20. * i_time), 0.6), 0.5);
