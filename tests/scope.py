...
@test_render(width=480, height=480, keyframes=5, tolerance=3, diff_threshold=0.005)
@ngl.scene()
...
def _get_histogram_func(mode):
    @test_render(width=320, height=240, keyframes=5, tolerance=5, diff_threshold=0.005)
    @ngl.scene()
    def scene_func(cfg):
...
def _get_waveform_func(mode):
    @test_render(width=320, height=240, keyframes=5, tolerance=3, diff_threshold=0.02)
    @ngl.scene()
    def scene_func(cfg):
        media = load_media("mire")
        cfg.duration = media.duration
...
@test_render(width=320, height=240, keyframes=5, tolerance=3, diff_threshold=0.15)
@ngl.scene()
def scope_rtt(cfg):
