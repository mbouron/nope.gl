...
static void vaapi_release_frame_resources(struct hwmap *hwmap)
{
    ...
    if (vaapi->surface_acquired) {
        for (size_t i = 0; i < NGLI_ARRAY_NB(vaapi->planes); i++) {
...
static int vaapi_map_frame(struct hwmap *hwmap, struct nmd_frame *frame)
{
    ...
    for (size_t i = 0; i < num_layers; i++) {
        ...
        const uint32_t object_index = vaapi->surface_descriptor.layers[i].object_index[0];
        ...
        const struct ngpu_texture_params texture_params = {
            ...
            .import_params = {
                ...
                .dma_buf = {
                    ...
                    .pitch          = vaapi->surface_descriptor.layers[i].pitch[0],
        ...
        int ret = ngpu_texture_init(vaapi->planes[i], &texture_params);
        if (ret < 0)
            return ret;
