...
static void customtexture_uninit(struct ngl_node *node)
{
    ...
    ngli_texture_freep(&s->texture_info.texture);
...
static int wrap_texture_gl(struct ngl_node *node, struct ngl_custom_texture_info *info)
{
    ...
    s->texture_info.texture = ngli_texture_create(gpu_ctx);
    if (!s->texture_info.texture)
        return NGL_ERROR_MEMORY;

    int ret = ngli_texture_gl_wrap(s->texture_info.texture, &wrap_params);
    if (ret < 0)
        return ret;
