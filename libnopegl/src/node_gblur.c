...
struct gblur_priv {
    ...
    struct pipeline *pl_blur_h;
    struct pipeline *pl_blur_v;
};
...
static int setup_pipeline(struct ngl_ctx *ctx, struct ngpu_pgcraft *crafter, struct pipeline *pipeline, const struct ngpu_rendertarget_layout *layout)
{
...
static int gblur_init(struct ngl_node *node)
{
    ...
    s->tmp_image = ngli_resource_create(NGLI_RESOURCE_IMAGE);
    if (!s->tmp_image)
        return NGL_ERROR_MEMORY;
    ...
    ret = ngli_pipeline_set_image_source(s->pl_blur_h, 0, s->image, NULL);
    if (ret < 0 && ret != NGL_ERROR_NOT_FOUND)
        return ret;
    ret = ngli_pipeline_set_image_source(s->pl_blur_v, 0, s->tmp_image, NULL);
    if (ret < 0 && ret != NGL_ERROR_NOT_FOUND)
        return ret;
    return 0;
}
...
static int resize(struct ngl_node *node)
{
    ...
    ngli_resource_clear(s->tmp_image);
...
static void gblur_pre_draw(struct ngl_node *node)
{
    ...
    ngli_resource_set_image(s->tmp_image, ngli_rtt_get_image(s->tmp, 0));
...
static void gblur_uninit(struct ngl_node *node)
{
    ...
    ngli_resource_freep(&s->tmp_image);
}
