...
struct gblur_opts {
    ...
    float bluriness;
    struct ngl_node *max_kernel_size_node;
...
static const struct node_param gblur_params[] = {
    ...
    {"max_kernel_size",   NGLI_PARAM_TYPE_U32, OFFSET(max_kernel_size_node), {.u32=127},
                          .flags=NGLI_PARAM_FLAG_ALLOW_NODE,
                          .desc=NGLI_DOCSTRING("maximum kernel size to be used internally, "
                          "a higher value increase the maximum blur amount at the cost of "
                          "performance, must be in the range [3," CONSTANT_TO_STR(MAX_KERNEL_SIZE) "]")},
    {NULL}
};
...
static int update_kernel(struct ngl_node *node)
{
    ...
    float bluriness = *(float *)ngli_node_get_data_ptr(o->bluriness_node, &o->bluriness);
    if (bluriness < 0.0)
        return NGL_ERROR_INVALID_ARG;

    uint32_t max_kernel_size = *(uint32_t *)ngli_node_get_data_ptr(o->max_kernel_size_node, &o->max_kernel_size);
    if (max_kernel_size < 3 || max_kernel_size > MAX_KERNEL_SIZE) {
        LOG(ERROR, "maximum kernel size \"%u\" must be in the range [3," CONSTANT_TO_STR(MAX_KERNEL_SIZE) "]", max_kernel_size);
        return NGL_ERROR_INVALID_ARG;
    }

    if (s->bluriness == bluriness && s->max_kernel_size == max_kernel_size)
        return 0;

    s->bluriness = bluriness;

    /* Round to the previous odd unsigned integer as the kernel size must be odd */
    s->max_kernel_size = (max_kernel_size - 1) | 0x1;
    s->max_radius_size = s->max_kernel_size - 1;

    float radius_f = NGLI_CLAMP(bluriness, 0.f, 1.f) * (float)s->max_radius_size;
    int32_t radius = NGLI_MIN((int32_t)ceil(radius_f), s->max_radius_size);
     ...
     */
    float sigma = (radius_f + 1.f) / sqrtf(-2.f * logf(1e-3f));
