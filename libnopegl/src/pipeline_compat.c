...
static int init_blocks_buffers(struct pipeline_compat *s, const struct pipeline_compat_params *params)
{
    ...
    for (size_t i = 0; i < NGLI_PROGRAM_SHADER_NB; i++) {
        const size_t block_size = ngli_block_get_size(&compat_info->ublocks[i], 0);
        if (!block_size)
            continue;
        ...
        ngli_pipeline_compat_update_buffer(s, compat_info->uindices[i], buffer, 0, buffer->size);
    }
...
int ngli_pipeline_compat_init(struct pipeline_compat *s, const struct pipeline_compat_params *params)
{
    ...
    ret = ngli_bindgroup_layout_params_copy(&s->bindgroup_layout_params, &params->layout);
    if (ret < 0)
        return ret;
    ...
    ret = ngli_bindgroup_layout_init(s->bindgroup_layout, &params->layout);
    if (ret < 0)
        return ret;
    ...
    if (nb_buffers) {
        s->buffers = ngli_calloc(nb_buffers, sizeof(struct buffer_binding));
    ...
    if (nb_textures) {
        s->textures = ngli_calloc(nb_textures, sizeof(struct texture_binding));
...
static int update_texture(struct pipeline_compat *s, int32_t index, const struct texture_binding *binding)
{
    ...
    ngli_assert(index >= 0);
    ngli_assert(index < s->nb_textures);
    ngli_assert(index < s->bindgroup_layout_params.nb_textures);
...
void ngli_pipeline_compat_update_texture_info(struct pipeline_compat *s, const struct pgcraft_texture_info *info)
{
    ...
    for (size_t i = 0; i < NGLI_ARRAY_NB(samplers); i++) {
        ...
        ret &= update_texture(s, index, &bindings[sampler]);
    };
...
static void prepare_bindgroup(struct pipeline_compat *s)
{
    if (!s->updated)
        return;
    ...
    for (size_t i = 0; i < s->nb_textures; i++)
        ngli_bindgroup_set_texture(s->cur_bindgroup, (int32_t)i, &s->textures[i]);

    for (size_t i = 0; i < s->nb_buffers; i++)
        ngli_bindgroup_set_buffer(s->cur_bindgroup, (int32_t)i, &s->buffers[i]);
}
