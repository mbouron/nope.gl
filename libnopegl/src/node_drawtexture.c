...
static int drawtexture_init(struct ngl_node *node)
{
    ...
    int ret = ngli_filterschain_init(s->filterschain, "source_texture", base_frag, 0);
    if (ret < 0)
        return ret;
                             ...
                             const struct ngpu_rendertarget_layout *rendertarget_layout)
{
    ...
    ret = ngli_pipeline_set_image_source(desc->pipeline, 0, texture_info->resource);
    if (ret < 0 && ret != NGL_ERROR_NOT_FOUND)
        return ret;
...
static void drawtexture_draw(struct ngl_node *node)
{
    struct drawtexture_priv *s = node->priv_data;
    ...
    struct drawtexture_vert_block vert_data;
    ...
    if (s->frag_block_index >= 0) {
        ...
        const struct drawtexture_frag_block frag_data = {
