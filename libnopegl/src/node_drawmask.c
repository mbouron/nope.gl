...
static int drawmask_init(struct ngl_node *node)
{
    ...
    ngpu_block_desc_add_field(&s->frag_block_desc, "inverted", NGPU_TYPE_BOOL, 0);
                            ...
                            const struct ngpu_rendertarget_layout *rendertarget_layout)
{
    ...
    ngli_assert(vert_size == sizeof(struct drawmask_vert_block));
    ngli_assert(frag_size >= sizeof(struct drawmask_frag_block));
    ...
    if (ngli_darray_try_push(&desc->reframing_nodes, o->content) < 0 ||
        ngli_darray_try_push(&desc->reframing_nodes, o->mask) < 0)
        return NGL_ERROR_MEMORY;
...
static void drawmask_draw(struct ngl_node *node)
{
    ...
    struct drawmask_vert_block vert_data;
    ...
    if (s->frag_block_index >= 0) {
        ...
        const struct drawmask_frag_block frag_data = {
