...
static int texture_prefetch(struct ngl_node *node)
{
    ...
    ngli_resource_set_image(i->resource, &i->image);
    return 0;
}
...
static int handle_media_frame(struct ngl_node *node)
{
    ...
    ngli_resource_clear(i->resource);
...
static void texture_pre_draw(struct ngl_node *node)
{
    ...
    if (!o->forward_transforms) {
        if (ngli_darray_try_push(&ctx->modelview_matrix_stack, ctx->default_modelview_matrix) < 0 ||
            ngli_darray_try_push(&ctx->projection_matrix_stack, ctx->default_projection_matrix) < 0)
            return;
    }
...
static void texture_uninit(struct ngl_node *node)
{
    struct texture_info *i = node->priv_data;
    ngli_resource_clear(i->resource);
    ngli_resource_freep(&i->resource);
}
...
static int texture2d_init(struct ngl_node *node)
{
    ...
    i->resource = ngli_resource_create(NGLI_RESOURCE_IMAGE);
    if (!i->resource)
        return NGL_ERROR_MEMORY;
    ...
    if (i->params.width  > max_dimension ||
        i->params.height > max_dimension) {
...
static void texture2d_get_rendertarget_layout(const struct ngl_node *node,
...
const struct node_class ngli_texture2d_class = {
    ...
    .name      = "Texture2D",
    .init      = texture2d_init,
    .get_rendertarget_layout = texture2d_get_rendertarget_layout,
