...
static int drawhistogram_init(struct ngl_node *node)
{
    ...
    int ret = ngli_filterschain_init(s->filterschain, "source_histogram", source_histogram_frag, 0);
    if (ret < 0)
        return ret;
                                 ...
                                 const struct ngpu_rendertarget_layout *rendertarget_layout)
{
    ...
    ngli_assert(vert_size == sizeof(struct drawhistogram_vert_block));
    ngli_assert(frag_size >= sizeof(struct drawhistogram_frag_block));
    ...
    ret = ngli_pipeline_set_buffer_source(desc->pipeline, stats_index, block_info->resource, 0, NGPU_BUFFER_WHOLE_SIZE);
    if (ret < 0 && ret != NGL_ERROR_NOT_FOUND)
        return ret;
...
static void drawhistogram_draw(struct ngl_node *node)
{
    ...
    if (s->frag_block_index >= 0) {
        ...
        const struct drawhistogram_frag_block frag_data = {
...
static void drawhistogram_release(struct ngl_node *node)
{
    struct drawhistogram_priv *s = node->priv_data;
    ngli_pipeline_discard_resources(s->pipeline_desc.pipeline);
}

static void drawhistogram_unprepare(struct ngl_node *node)
{
    struct drawhistogram_priv *s = node->priv_data;
    struct pipeline_desc *desc = &s->pipeline_desc;
...
static void drawhistogram_uninit(struct ngl_node *node)
{
    struct drawhistogram_priv *s = node->priv_data;
...
static uint32_t drawhistogram_get_renderpass_usage(const struct ngl_node *node)
{
    const struct drawhistogram_opts *o = node->opts;
    return ngli_graphics_state_get_renderpass_usage(&o->state);
}

const struct node_class ngli_drawhistogram_class = {
    .id        = NGL_NODE_DRAWHISTOGRAM,
    .category  = NGLI_NODE_CATEGORY_DRAW,
    .name      = "DrawHistogram",
    .init      = drawhistogram_init,
    .prepare   = drawhistogram_prepare,
    .unprepare = drawhistogram_unprepare,
