...
int ngli_params_set_dict(uint8_t *dstp, const struct node_param *par, const char *name, struct ngl_node *node)
{
    ...
    if (!*hmapp) {
        *hmapp = ngli_hmap_try_create(NGLI_HMAP_TYPE_STR);
        if (!*hmapp)
            return NGL_ERROR_MEMORY;
        ngli_hmap_set_free_func(*hmapp, node_hmap_free, NULL);
    }

    ret = ngli_hmap_try_set_str(*hmapp, name, node);
    if (ret < 0)
        return ret;
    if (node)
        ngl_node_ref(node);
    ...
    for (size_t i = 0; params[i].key; i++) {
        ...
        switch (par->type) {
            ...
            case NGLI_PARAM_TYPE_NODELIST: {
                struct ngli_node_darray *array = (struct ngli_node_darray *)parp;
                for (size_t j = 0; j < array->count; j++)
                    ngl_node_unrefp(&array->data[j]);
