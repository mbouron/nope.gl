...
static const struct ngpu_block_field *get_block_field(const struct ngpu_block_field *fields, size_t nb_fields, const char *name)
{
...
static int buffer_init_from_block(struct ngl_node *node)
{
    ...
    const struct ngpu_block_field *fi = get_block_field(block->fields, block->nb_fields, o->block_field);
    if (!fi) {
...
static int buffer_init(struct ngl_node *node)
{
    ...
    if (s->buf.block) {
        ...
        s->buf.resource = ngli_buffer_resource_ref(block_info->resource);
    } else {
        s->buf.resource = ngli_buffer_resource_create();
        if (!s->buf.resource)
            return NGL_ERROR_MEMORY;
    }
...
static int buffer_init_resources(struct ngl_node *node)
{
    ...
    ret = ngpu_buffer_upload(buffer, info->data, 0, info->data_size);
    if (ret < 0) {
        ngpu_buffer_freep(&buffer);
        return ret;
    }
...
static void buffer_uninit(struct ngl_node *node)
{
    ...
    if (o->block)
        ngli_buffer_resource_freep(&s->buf.resource);
    else
