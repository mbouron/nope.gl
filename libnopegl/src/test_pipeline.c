...
static struct pipeline *create_pipeline(struct fixture *f, struct ngpu_pgcraft *crafter)
{
    struct pipeline *pipeline = ngli_pipeline_create(f->gpu);
    CHECK(pipeline);
...
static void test_buffer_sources(struct fixture *f)
{
    ...
    struct pipeline *a = create_pipeline(f, crafter);
...
static void test_image_sources(struct fixture *f)
{
    ...
    *selected = image;
    ...
    check_pixel(f, 1, (const uint8_t[]){64, 64, 64, 255});
    CHECK(image.coordinates_matrix.m[12] == .5f);

    /* Same planes, different metadata and consumer transform within one frame. */
    begin_frame(f);
    viewport(f, 0);
    image.ts = .5f;
    ngli_resource_set_image(source_a, &image);
