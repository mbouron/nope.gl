...
static void publish_buffer(struct fixture *f, struct buffer_resource *resource, size_t size, const void *data, uint32_t usage)
{
...
static void test_buffer_sources(struct fixture *f)
{
    ...
    struct buffer_resource *owner = ngli_buffer_resource_create();
    struct buffer_resource *indices = ngli_buffer_resource_create();
    CHECK(owner && indices);
    ...
    CHECK(ngli_pipeline_set_buffer_source(a, -1, NULL, 0, 0) == NGL_ERROR_NOT_FOUND);
    CHECK(ngli_pipeline_set_buffer_source(a, 99, owner, 0, NGPU_BUFFER_WHOLE_SIZE) == NGL_ERROR_INVALID_ARG);
    CHECK(ngli_pipeline_set_vertex_source(a, 99, owner) == NGL_ERROR_INVALID_ARG);
    struct ngli_pipeline *pipelines[] = {a, b};
    for (size_t i = 0; i < 2; i++) {
        CHECK(ngli_pipeline_set_buffer_source(pipelines[i], block_index, owner, 0, NGPU_BUFFER_WHOLE_SIZE) == 0);
    ...
    data[1] = 0; data[2] = 1;
    CHECK(ngpu_buffer_upload(ngli_buffer_resource_get(owner), data, 0, sizeof(data)) == 0);
    begin_frame(f);
    viewport(f, 0);
    ...
    CHECK(ngli_pipeline_set_buffer_source(a, block_index, owner, SIZE_MAX, 16) == 0);
    begin_frame(f);
    CHECK(ngli_pipeline_draw_indexed(a, NULL, 3, 1) < 0);
    CHECK(ngli_pipeline_draw_indexed(a, NULL, 3, 1) < 0);
    CHECK(ngpu_ctx_get_frame_stats(f->gpu)->draw_calls == 0);
    CHECK(ngli_pipeline_set_buffer_source(a, block_index, owner, 0, NGPU_BUFFER_WHOLE_SIZE) == 0);
    viewport(f, 0);
    CHECK(ngli_pipeline_draw_indexed(a, NULL, 3, 1) == 0);
    end_frame(f);
    check_pixel(f, 0, (const uint8_t[]){0, 0, 255, 255});
    ...
    /*
     * Withdrawing a publication fails the next execution, but the bindgroups
     * keep the references they were last updated with and the failed execution
     * never updates them again. Discarding must therefore release the
     * allocation: both pipelines bind this holder twice (block and vertices).
     */
    ngli_buffer_resource_set(owner, NULL);
    begin_frame(f);
    CHECK(ngli_pipeline_draw_indexed(a, NULL, 3, 1) < 0);
    CHECK(ngli_pipeline_draw_indexed(b, NULL, 3, 1) < 0);
    CHECK(ngpu_ctx_get_frame_stats(f->gpu)->draw_calls == 0);
    ...
    /* Both cached owner allocations were released; only the index buffer remains. */
    CHECK(ngpu_ctx_get_memory_stats(f->gpu)->buffer_count == initial_buffers + 1);
    data[0] = 1; data[2] = 0;
    ...
    CHECK(ngli_pipeline_set_buffer_source(a, block_index, NULL, 0, 0) == 0);
    CHECK(ngli_pipeline_update_buffer(a, block_index, ngli_buffer_resource_get(owner), 0, 0) == NGL_ERROR_INVALID_ARG);
    CHECK(ngli_pipeline_update_buffer(a, block_index, ngli_buffer_resource_get(owner), 0, NGPU_BUFFER_WHOLE_SIZE) == 0);
    CHECK(ngli_pipeline_set_buffer_source(a, block_index, owner, 0, NGPU_BUFFER_WHOLE_SIZE) == NGL_ERROR_INVALID_USAGE);
    ngli_pipeline_discard_resources(a);
    begin_frame(f);
    CHECK(ngli_pipeline_draw_indexed(a, NULL, 3, 1) < 0);
    CHECK(ngli_pipeline_update_buffer(a, block_index, ngli_buffer_resource_get(owner), 0, 16) == 0);
    viewport(f, 0);
    CHECK(ngli_pipeline_draw_indexed(a, NULL, 3, 1) == 0);
    end_frame(f);

    ngli_buffer_resource_freep(&owner);
    ngli_buffer_resource_freep(&indices);
    ...
    /*
     * Cycling through discard and redraw must not accumulate GPU resources.
     * Each discard destroys the bindgroups, so their backing descriptor sets
     * have to be recycled rather than taken from the pool again.
     */
    const uint64_t cycle_buffers = ngpu_ctx_get_memory_stats(f->gpu)->buffer_count;
    const uint64_t cycle_textures = ngpu_ctx_get_memory_stats(f->gpu)->texture_count;
    ...
    check_pixel(f, 0, (const uint8_t[]){255, 0, 0, 255});
    CHECK(ngpu_ctx_get_memory_stats(f->gpu)->buffer_count == cycle_buffers);
    CHECK(ngpu_ctx_get_memory_stats(f->gpu)->texture_count == cycle_textures);
...
static struct ngli_image *create_image(const struct ngli_image_params *params)
{
...
static void test_image_sources(struct fixture *f)
{
    ...
    image_params.coordinates_matrix.m[12] = .5f;
    struct ngli_image *image = create_image(&image_params);
    struct ngli_image *selected = create_image(&image_params);
    struct image_resource *source_a = ngli_image_resource_create();
    struct image_resource *source_b = ngli_image_resource_create();
    struct texture_resource *raw = ngli_texture_resource_create();
    CHECK(source_a && source_b && raw);
    ngli_image_resource_set(source_a, image);
    ngli_image_resource_set(source_b, selected);
    ngli_texture_resource_set(raw, texture);
    ...
    CHECK(ngli_pipeline_update_buffer(a, ngpu_pgcraft_get_texture_infos(crafter).block_index, NULL, 0, 0) == NGL_ERROR_INVALID_USAGE);
    ...
    CHECK(ngli_image_get_params(image)->coordinates_matrix.m[12] == .5f);
    ...
    viewport(f, 1);
    image_params.ts = .75f;
    image_params.coordinates_matrix.m[12] = .25f;
    ngli_image_unrefp(&image);
    image = create_image(&image_params);
    ngli_image_resource_set(source_a, image);
    CHECK(ngli_pipeline_draw(a, &execution, 3, 1, 0) == 0);
    ...
    image_params.layout = NGLI_IMAGE_LAYOUT_NV12;
    image_params.planes[1] = chroma;
    ngli_image_unrefp(&image);
    image = create_image(&image_params);
    begin_frame(f);
    viewport(f, 0);
    ngli_image_resource_set(source_a, image);
    CHECK(ngli_pipeline_draw(a, &execution, 3, 1, 0) == 0);
    image_params.layout = NGLI_IMAGE_LAYOUT_DEFAULT;
    image_params.planes[1] = NULL;
    ngli_image_unrefp(&image);
    image = create_image(&image_params);
    viewport(f, 1);
    ngli_image_resource_set(source_a, image);
    ...
    /* Publication retains the image after the producer drops its reference. */
    ngli_image_resource_set(source_b, NULL);
    ngli_image_unrefp(&selected);
    image_params.ts = .5f;
    image_params.coordinates_matrix.m[12] = .5f;
    selected = create_image(&image_params);
    ngli_image_resource_set(source_b, selected);
    ngli_image_unrefp(&selected);
    begin_frame(f);
    viewport(f, 0);
    ...
    /* The source, image and plane outlive every producer reference. */
    ngli_image_resource_set(source_a, ngli_image_resource_get(source_a));
    ngli_image_resource_freep(&source_a);
    ngli_image_unrefp(&image);
    ngli_texture_resource_freep(&raw);
    ...
    ngli_image_resource_freep(&source_b);
    ngli_pipeline_freep(&a);
    ngli_pipeline_freep(&b);
...
static void test_texture_sources(struct fixture *f)
{
    ...
    struct texture_resource *source = ngli_texture_resource_create();
    CHECK(source);
    ...
    ngli_texture_resource_set(source, texture);
    ngpu_texture_freep(&texture);
    begin_frame(f);
    viewport(f, 0);
    ...
    ngli_texture_resource_set(source, ngli_texture_resource_get(source));
    ngli_texture_resource_freep(&source);
...
static void test_dynamic_ranges(struct fixture *f)
{
    ...
    ngpu_block_desc_init(f->gpu, &block, NGPU_BLOCK_LAYOUT_STD140);
    CHECK(ngpu_block_desc_add_field(&block, "color", NGPU_TYPE_VEC4, 0) >= 0);
    const struct ngpu_pgcraft_block block_descs[] = {
        {.name = "params", .instance_name = "", .stage = NGPU_PROGRAM_STAGE_FRAG,
         .type = NGPU_TYPE_UNIFORM_BUFFER_DYNAMIC, .block = &block},
        {.name = "mask", .instance_name = "mask", .stage = NGPU_PROGRAM_STAGE_FRAG,
         .type = NGPU_TYPE_UNIFORM_BUFFER_DYNAMIC, .block = &block},
    };
    const struct ngpu_pgcraft_params params = {
        .vert_base = "void main() { vec2 p = vec2((ngl_vertex_index << 1) & 2, ngl_vertex_index & 2); ngl_out_pos = vec4(p * 2.0 - 1.0, 0.0, 1.0); }",
        .frag_base = "void main() { ngl_out_color = color * mask.color; }",
        .blocks = block_descs, .nb_blocks = 2,
    };
    struct ngpu_pgcraft *crafter = ngpu_pgcraft_create(f->gpu);
    CHECK(crafter && ngpu_pgcraft_craft(crafter, &params) == 0);
    /* Array indices need not follow shader binding order. */
    const struct ngpu_bindgroup_layout_desc layout = ngpu_pgcraft_get_bindgroup_layout_desc(crafter);
    CHECK(layout.nb_buffers == 2);
    const struct ngpu_bindgroup_layout_entry first = layout.buffers[0];
    layout.buffers[0] = layout.buffers[1];
    layout.buffers[1] = first;
    CHECK(layout.buffers[0].binding > layout.buffers[1].binding);
    struct ngli_pipeline *pipeline = create_pipeline(f, crafter);
    struct buffer_resource *owner = ngli_buffer_resource_create();
    CHECK(owner);
    ...
    memset(data, 0, size);
    memcpy(data, (const float[]){1, 1, 1, 1}, 16);
    struct ngpu_buffer *mask = ngpu_buffer_create(f->gpu);
    CHECK(mask && ngpu_buffer_init(mask, size, NGPU_BUFFER_USAGE_UNIFORM_BUFFER_BIT | NGPU_BUFFER_USAGE_TRANSFER_DST_BIT) == 0);
    CHECK(ngpu_buffer_upload(mask, data, 0, size) == 0);
    const int32_t mask_index = ngpu_pgcraft_get_block_index(crafter, "mask", NGPU_PROGRAM_STAGE_FRAG);
    CHECK(ngli_pipeline_update_buffer(pipeline, mask_index, mask, 0, 16) == 0);
    free(data);
    const int32_t index = ngpu_pgcraft_get_block_index(crafter, "params", NGPU_PROGRAM_STAGE_FRAG);
    CHECK(ngli_pipeline_set_buffer_source(pipeline, index, owner, 0, 16) == 0);

    begin_frame(f);
    uint32_t offset = (uint32_t)alignment * 2;
    CHECK(ngli_pipeline_set_buffer_source(pipeline, index, owner, offset, 16) == 0);
    ...
    ngli_pipeline_discard_resources(pipeline);
    begin_frame(f);
    /* Discard restores source offsets, but direct bindings must be supplied again. */
    CHECK(ngli_pipeline_draw(pipeline, NULL, 3, 1, 0) < 0);
    CHECK(ngli_pipeline_update_buffer(pipeline, mask_index, mask, 0, 16) == 0);
    viewport(f, 0);
    CHECK(ngli_pipeline_draw(pipeline, NULL, 3, 1, 0) == 0);
...
static void test_required_storage_image(struct fixture *f)
{
    ...
    image_params.planes[0] = wrong_format;
    image = create_image(&image_params);
    ngli_image_resource_set(source, image);
    ngli_image_unrefp(&image);
    ngpu_ctx_advance_frame(f->gpu);
