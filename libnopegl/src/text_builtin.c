...
static int text_builtin_init(struct text *text)
{
    ...
    ngli_resource_set_texture(text->curve_texture, ngli_slug_get_curve_texture(atlas->slug));
    ngli_resource_set_texture(text->band_texture, ngli_slug_get_band_texture(atlas->slug));
...
static int text_builtin_set_string(struct text *text, const char *str, struct ngli_char_info_internal_darray *chars_dst)
{
    struct text_builtin *s = text->priv_data;
