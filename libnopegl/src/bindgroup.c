...
#define ARRAY_MEMDUP(dp, sp, name) do {                                 \
    size_t nb = (sp)->nb_##name;                                        \
    if (nb > 0) {                                                       \
        (dp)->name = ngli_memdup((sp)->name, nb * sizeof(*(dp)->name)); \
        if (!(dp)->name)                                                \
...
int ngli_bindgroup_layout_params_copy(struct bindgroup_layout_params *desc, const struct bindgroup_layout_params *src)
{
    ARRAY_MEMDUP(desc, src, textures);
    ARRAY_MEMDUP(desc, src, buffers);
...
{
    ARRAY_MEMDUP(s, params, textures);
    ARRAY_MEMDUP(s, params, buffers);
...
int ngli_bindgroup_set_texture(struct bindgroup *s, int32_t index, const struct texture_binding *binding)
{
    if (index == -1)
    ...
    return s->gpu_ctx->cls->bindgroup_set_texture(s, index, binding);
}

int ngli_bindgroup_set_buffer(struct bindgroup *s, int32_t index, const struct buffer_binding *binding)
{
    if (index == -1)
        return NGL_ERROR_NOT_FOUND;
    ...
    return s->gpu_ctx->cls->bindgroup_set_buffer(s, index, binding);
}
