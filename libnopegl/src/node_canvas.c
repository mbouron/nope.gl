...
static int canvas_swap_children(struct ngl_node *node, size_t from, size_t to)
{
    struct canvas_priv *s = node->priv_data;
...
#define OFFSET(x) offsetof(struct canvas_opts, x)
static const struct node_param canvas_params[] = {
    {
        .key       = "children",
        .type      = NGLI_PARAM_TYPE_NODELIST,
        .offset    = OFFSET(children),
        .flags     = NGLI_PARAM_FLAG_ALLOW_LIVE_CHANGE,
        .swap_func = canvas_swap_children,
...
static int canvas_init(struct ngl_node *node)
{
    struct canvas_priv *s = node->priv_data;
...
static int canvas_prepare(struct ngl_node *node)
{
    struct ngl_ctx *ctx = node->ctx;
    struct canvas_priv *s = node->priv_data;
    const struct canvas_opts *o = node->opts;
...
static void canvas_draw(struct ngl_node *node)
{
    struct ngl_ctx *ctx = node->ctx;
    ...
    /* Save previous 2D state so nested Canvas (e.g. via Texture2D RTT) works */
    ...
    /* Initialize fresh stacks for this Canvas */
...
static void canvas_uninit(struct ngl_node *node)
{
    struct canvas_priv *s = node->priv_data;
...
const struct node_class ngli_canvas_class = {
    .id        = NGL_NODE_CANVAS,
    .name      = "Canvas",
    .priv_size = sizeof(struct canvas_priv),
    .init      = canvas_init,
    .prepare   = canvas_prepare,
