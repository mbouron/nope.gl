...
/*
 * Unsigned distance from the origin to a quadratic bezier curve adapted from
 * Inigo Quilez's sdBezier().
 ...
 * bandLoc points to the band location in the band texture.
 */
float minDistFromBand(ivec2 glyphLoc, ivec2 bandLoc, vec2 renderCoord)
{
    vec4 bandData = texelFetch(band_tex, bandLoc, 0);
    int count = int(bandData.x);
    ivec2 hBandLoc = CalcBandLoc(glyphLoc, int(bandData.y));
    float md = 1e10;
    for (int i = 0; i < count; i++) {
        vec4 clocData = texelFetch(band_tex, ivec2(hBandLoc.x + i, hBandLoc.y), 0);
...
vec2 SlugRender(vec2 renderCoord, vec4 bandTransform, ivec4 glyphData, bool needDist)
{
    ...
    if (needDist) {
        /*
         * Compute the minimum unsigned distance to all curves in the shape by
        ...
        for (int i = 0; i <= bandMax.y; i++) {
            ivec2 bandLoc = ivec2(glyphLoc.x + i, glyphLoc.y);
            minDist = min(minDist, minDistFromBand(glyphLoc, bandLoc, renderCoord));
        }
        int vBandBase = bandMax.y + 1;
        for (int i = 0; i <= bandMax.x; i++) {
            ivec2 bandLoc = ivec2(glyphLoc.x + vBandBase + i, glyphLoc.y);
            minDist = min(minDist, minDistFromBand(glyphLoc, bandLoc, renderCoord));
        }
    }
