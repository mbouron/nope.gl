...
struct drawdisplace_frag_block {
    float aspect;
    float _pad0[3];
};
...
static int drawdisplace_init(struct ngl_node *node)
{
    ...
    int ret = ngli_filterschain_init(s->filterschain, "source_displace", source_displace_frag, 0);
    if (ret < 0)
        return ret;
                                ...
                                const struct ngpu_rendertarget_layout *rendertarget_layout)
{
    ...
    ngli_assert(vert_size == sizeof(struct drawdisplace_vert_block));
    ngli_assert(frag_size >= sizeof(struct drawdisplace_frag_block));
    ...
    if (ngli_darray_push(&desc->reframing_nodes, o->source_node) < 0 ||
        ngli_darray_push(&desc->reframing_nodes, o->displacement_node) < 0)
        return NGL_ERROR_MEMORY;
...
static void drawdisplace_draw(struct ngl_node *node)
{
    ...
    struct drawdisplace_vert_block vert_data;
    ...
    if (s->frag_block_index >= 0) {
        ...
        const struct drawdisplace_frag_block frag_data = {
...
static uint32_t drawdisplace_get_renderpass_usage(const struct ngl_node *node)
{
    const struct drawdisplace_opts *o = node->opts;
    return ngli_graphics_state_get_renderpass_usage(&o->state);
}

const struct node_class ngli_drawdisplace_class = {
    .id        = NGL_NODE_DRAWDISPLACE,
