...
static int vk_init(struct ngpu_ctx *s)
{
    ...
    if ((s->params.platform == NGPU_PLATFORM_XLIB || s->params.platform == NGPU_PLATFORM_WAYLAND) &&
        ngpu_vkcontext_has_extensions(vk, NGLI_ARRAY_NB(dmabuf_required_extensions), dmabuf_required_extensions, 1))
    ...
    if (s->params.platform == NGPU_PLATFORM_ANDROID &&
        ngpu_vkcontext_has_extensions(vk, NGLI_ARRAY_NB(ahb_required_extensions), ahb_required_extensions, 1))
