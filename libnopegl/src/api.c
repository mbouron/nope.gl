...
NGL_API int ngl_get_nodes_at_point(struct ngl_ctx *s, const float *point, size_t *nb_nodesp, struct ngl_node ***nodesp)
{
    ...
    *nodesp = NULL;
    ...
    for (size_t i = 0; i < ngli_darray_count(&s->bounding_box_nodes); i++) {
        ...
        const struct draw_info *draw_info = bounding_box_node->priv_data;

        /* Broadphase: pixel-space AABB */
        if (!ngli_aabb_intersect_point(&draw_info->screen_aabb, pixel_point))
            continue;

        /* Narrow phase: inverse-transform point into local space */
        NGLI_ALIGNED_MAT(inv);
        ngli_mat4_inverse(inv, draw_info->transform_matrix);
        NGLI_ALIGNED_VEC(local_point);
        ngli_mat4_mul_vec4(local_point, inv, pixel_point);

        if (ngli_aabb_intersect_point(&draw_info->aabb, local_point)) {
