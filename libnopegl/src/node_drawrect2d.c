...
static int drawrect2d_init(struct ngl_node *node)
{
    ...
    if (fill_paint->glsl_header)
        ngli_bstr_printf(bstr, "%s\n", fill_paint->glsl_header);
    if (stroke_paint && stroke_paint->glsl_header &&
        (!fill_paint->glsl_header || strcmp(fill_paint->glsl_header, stroke_paint->glsl_header)))
        ngli_bstr_printf(bstr, "%s\n", stroke_paint->glsl_header);
    ngli_bstr_print(bstr, fill_paint->glsl[PAINT_SHADER_ROLE_FILL]);
    if (fill_paint->color_output_count) {
        ngli_bstr_print(bstr, "void main() { ngli_colors(ngli_uv, ngli_tex_coord); }\n");
    } else {
        if (stroke_paint)
            ngli_bstr_print(bstr, stroke_paint->glsl[PAINT_SHADER_ROLE_STROKE]);
        else
    ...
    const size_t nb_stroke_custom_uniforms = stroke_paint && stroke_paint != fill_paint ? stroke_paint->custom_uniforms.count : 0;
    ...
    if (has_user_uniforms) {
        ...
        for (size_t i = 0; i < nb_fill_uniforms; i++) {
            ...
            ngli_stroke2d_get_paint_resource_name(name, sizeof(name), PAINT_SHADER_ROLE_FILL, ud->name);
        ...
        for (size_t i = 0; i < nb_stroke_uniforms; i++) {
            ...
            char name[NGPU_ID_LEN];
            ngli_stroke2d_get_paint_resource_name(name, sizeof(name), PAINT_SHADER_ROLE_STROKE, ud->name);
            const int field_idx = ngpu_block_desc_add_field(&s->user_block_desc, name, ud->type, 0);
            if (field_idx < 0)
                return field_idx;
            const struct prebuilt_uniform pu = {
    ...
    if (stroke_paint && stroke_paint != fill_paint) {
...
static void drawrect2d_draw(struct ngl_node *node)
{
    ...
    /* Fill and push static fragment block to staging buffer */
    {
        ...
        frag_data.content_wrap  = fill_opts->wrap;
