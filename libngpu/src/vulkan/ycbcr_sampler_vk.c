...
{
    struct ngpu_ycbcr_sampler_vk *s = ngpu_try_calloc(1, sizeof(*s));
    if (!s)
        return NULL;
...
VkResult ngpu_ycbcr_sampler_vk_init(struct ngpu_ycbcr_sampler_vk *s, const struct ngpu_ycbcr_sampler_vk_params *params)
{
    ...
    const VkSamplerYcbcrConversionCreateInfo sampler_ycbcr_info = {
    ...
    VkResult res = vk->funcs.CreateSamplerYcbcrConversion(vk->device, &sampler_ycbcr_info, 0, &s->conv);
    if (res != VK_SUCCESS) {
